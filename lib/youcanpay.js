...
// la laisser dans un état "à vérifier manuellement" (voir api/payments/youcanpay/webhook.js).
...
async function initPayment(order, req) {
  ...
  form.set("success_url", origin + "/api/payments/youcanpay/return?ref=" + encodeURIComponent(order.ref));
  form.set("error_url", origin + "/api/payments/youcanpay/return?ref=" + encodeURIComponent(order.ref) + "&failed=1");
