...
function requireManager(req, res) {
  ...
  // Une session émise avant l'ajout de staffRole (compte déjà connecté au moment du
  // déploiement, cookie signé 14 jours) n'a pas ce champ -- traité comme "manager" par
  // défaut, cohérent avec le DEFAULT 'manager' de la colonne en base (voir
  // ensureStaffColumns dans api/auth/admin.js) et avec whoami qui applique déjà ce même
  // repli côté client. Sans ce repli ici, le compte gérant existant se voyait refuser ses
  // propres actions (ex. créer un code promo) tant qu'il ne s'était pas reconnecté.
  var staffRole = session.staffRole || "manager";
...
function requirePermission(req, res, panelKey) {
  var session = requireAdmin(req, res);
  if (!session) return null;
  var staffRole = session.staffRole || "manager"; // voir note dans requireManager ci-dessus
  if (staffRole === "manager") return session;
